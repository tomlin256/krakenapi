...
**Done**: implemented via [plan 004](004-step-6-binance-rest-private.md) in five checkpoint commits (6.1–6.5: `145bf70`, `c8e42c4`, `7c2db46`, `050b82d`, `8b63fa9`). All 7 request types + response/row structs live in `include/exchange/binance/rest_api.hpp`, inheriting the **local** `exchange::binance::rest::TypedPrivateRequest<R>` with credential-free `build()` — `execute(req, auth)` signs via `BinanceAuth` afterwards (plan 004 decision 1). POST params go body-side, GET/DELETE params query-side, matching `BinanceAuth::sign`'s signed-payload contract (decision 2). Enum mapping is canonical-where-total: `side`/`timeInForce`/`status` map to `exchange::` enums via new converters in `include/exchange/binance/types.hpp`; response `type` stays a raw wire string because `LIMIT_MAKER` has no canonical value (decision 3) — this resolves the line above about "Binance-only types like STOP_LOSS_LIMIT": `STOP_LOSS_LIMIT` *is* canonical (`StopLossLimit`); only `LIMIT_MAKER` is genuinely unmappable. Canonical `TimeInForce` gained `FOK`, with Kraken's converter throwing on it (decision 4). Order quantities/prices in requests are caller-formatted decimal strings (decision 6). Fixtures in `tests/unit/binance_account_example_json.hpp` (synthetic — credential-gated endpoints, shapes verbatim from the appendix); new binaries `test_binance_types` + `test_binance_client` (signed GET/POST/DELETE round-trips with recomputed signatures) alongside extended request/response tests — full suite 211 → 255 green. No example program (per the testing-strategy appendix, examples are public-only). Out of scope per plan 004 decision 9: `omitZeroBalances`, strategy/STP request params, `cancelRestrictions`, full OCO rows, orderList endpoints.
